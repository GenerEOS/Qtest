...
describe('account test', () => {
  ...
  describe('Increase time to specific time point', function () {
    it('should increase time to able to add new epoch', async () => {
      const currentBlockTimeMilliSecond = blockTimeToMs((await chain.getInfo()).head_block_time);
      const twoHoursAhead = new Date(currentBlockTimeMilliSecond + 2 * 60 * 60 * 1000);
      const oneDayAhead = new Date(currentBlockTimeMilliSecond + 1 * 24 * 60 * 60 * 1000);
      const twoDaysAhead = new Date(currentBlockTimeMilliSecond + 2 * 24 * 60 * 60 * 1000);
      const oneMonthAhead = new Date(currentBlockTimeMilliSecond + 30 * 24 * 60 * 60 * 1000);
      ...
      expect(epochs.rows[0].epoch_id).toBe(0);
      expect(epochs.rows[0].start_at).toBe(rsp.processed.block_time);
      expect(epochs.rows[0].end_at).toBe(oneDayAhead.toISOString().slice(0, -1));
      ...
      expect(epochs.rows[epochs.rows.length - 1].epoch_id).toBe(1);
      expect(epochs.rows[epochs.rows.length - 1].start_at).toBe(rsp.processed.block_time);
      expect(epochs.rows[epochs.rows.length - 1].end_at).toBe(twoDaysAhead.toISOString().slice(0, -1));
    }, 50000);
  });
});
