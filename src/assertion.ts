...
export function getAllActions(actionTraces: ActionTrace[]): { account: string, name: string, data?: object, authorization?: Authorization[] }[] {
  let actions = [];
...
export function expectAction(transaction: TransactResult, expectedAction: { account: string, name: string, data?: object, authorization?: Authorization[] }): boolean {
  const allActions = getAllActions(transaction.processed.action_traces);
  for (const action of allActions) {
    if (action.account === expectedAction.account && action.name === expectedAction.name) {
      if (expectedAction.data) {
        if (JSON.stringify(expectedAction.data) !== JSON.stringify(action.data)) {
          throw new Error(`Data of action ${action.account}:${action.name} mismatch`);
        }
      }

      if (expectedAction.authorization) {
        if (JSON.stringify(expectedAction.authorization) !== JSON.stringify(action.authorization)) {
          throw new Error(`Authorization of action ${action.account}:${action.name} mismatch`);
        }
      }
